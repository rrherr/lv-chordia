...
Reads: chordnet_ismir_naive.py, mir/nn/network.py, extractors/xhmm_ismir.py, settings.py, audio_utils.py, device_utils.py
"""
...
def probabilities(ensemble: List[NetworkInterface], audio: np.ndarray) -> List[np.ndarray]:
    ...
    probs = []
    for net in ensemble:
        print('Inference: %s' % net.save_name, file=sys.stderr)
...
def recognize_with_ensemble(
    ...
    try:
        audio, _ = librosa.load(actual_audio_path, sr=SAMPLE_RATE, mono=True)
